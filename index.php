        ...
        if ( isset($_GET['ord'])) {
            switch ( $_GET['ord']) {
                case "id"  : 
                    ...
                    $tvalores = $db->ordenarClientes('id',$posini,FPAG);
                    require_once "app/views/list.php";
                    break;
                case "nombre":
                    $db = AccesoDatos::getModelo();
                    $posini = $_SESSION['posini'];
                    $tvalores = $db->ordenarClientes('first_name',$posini,FPAG);
                    require_once "app/views/list.php";
                    break;
                case "email":
                    $db = AccesoDatos::getModelo();
                    $posini = $_SESSION['posini'];
                    $tvalores = $db->ordenarClientes('email',$posini,FPAG);
                    require_once "app/views/list.php";
                    break;
                case "gen":
                    $db = AccesoDatos::getModelo();
                    $posini = $_SESSION['posini'];
                    $tvalores = $db->ordenarClientes('gender',$posini,FPAG);
                    require_once "app/views/list.php";
                    break;
                case "ip":
                    $db = AccesoDatos::getModelo();
                    $posini = $_SESSION['posini'];
                    $tvalores = $db->ordenarClientes('ip_address',$posini,FPAG);
                    require_once "app/views/list.php";
                    break;
                case "tel":
                    $db = AccesoDatos::getModelo();
                    $posini = $_SESSION['posini'];
                    $tvalores = $db->ordenarClientes('telefono',$posini,FPAG);
                    require_once "app/views/list.php";   
            }
            $_SESSION['posini'] = $posAux;
        }
